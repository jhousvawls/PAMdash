  ...
  const calculateQuestMetrics = (person) => {
    const quests = [
      ...
      },
      {
        ...
        type: "opportunities",
        weight: 15, // 15% of total score (assuming this is what the duplicate "Opps Passed %" should be)
        maxPoints: 150
      }
    ];
...
Alex Kim,75,71,95,12,16,133,4,5,125,30000,38000,127,15000,19000,127`;
  ...
  const getQuestStatusColor = (status, completionRate) => {
    ...
    if (completionRate >= 50) return 'bg-teal-500';
    return 'bg-red-500';
  };

  const getQuestStatusIcon = (status) => {
    if (status === 'completed') return <CheckCircle className="w-5 h-5 text-green-600" />;
    if (status === 'almost') return <AlertCircle className="w-5 h-5 text-yellow-600" />;
  ...
  const getLevelIcon = (level) => {
    switch (level) {
      ...
      case 'Quest Master': return <Trophy className="w-6 h-6 text-teal-500" />;
      case 'Quest Warrior': return <Medal className="w-6 h-6 text-orange-500" />;
      case 'Quest Explorer': return <Star className="w-6 h-6 text-blue-500" />;
  ...
  const renderQuestCard = (person) => (
    <div key={person.id} className="bg-white rounded-xl shadow-lg p-6 border-2 border-gray-200 hover:border-blue-300 transition-all duration-300">
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-4">
          <div className="w-14 h-14 bg-gradient-to-r from-teal-500 to-blue-600 rounded-full flex items-center justify-center text-white font-bold text-xl">
            {person.avatar}
          </div>
          <div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-teal-50 to-blue-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Complete your sales quests and become a legendary sales warrior!</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8 border border-gray-100">
          ...
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-teal-400 transition-colors">
            ...
            <label htmlFor="csv-upload" className="cursor-pointer">
              ...
              <p className="text-lg text-gray-600 mb-2">Upload Sales Quest Data CSV</p>
          ...
          {uploadMessage && (
            <div className="mt-4 p-3 bg-teal-50 border border-teal-200 rounded-lg">
              <p className="text-teal-800">{uploadMessage}</p>
            </div>
          )}
        </div>
